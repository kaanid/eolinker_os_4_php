...
class ImportModule
{
    ...
    private function getSwaggerParamType($strType)
    {
        // 请求参数类型数组
        $param_type = array(
            'string' => '0',
        ...
        switch ($strType) {
            ...
            case "string" :
                ...
                break;
            case 'long' :
                $result = $param_type['long'];
                break;
            case 'float' :
            $result = $param_type['float'];
                break;
            case 'double' :
            $result = $param_type['double'];
                break;
            case 'byte' :
            $result = $param_type['byte'];
                break;
            case 'file' :
            $result = $param_type['file'];
                break;
            case 'date' :
            $result = $param_type['date'];
                break;
            case 'dateTime' :
            $result = $param_type['dateTime'];
                break;
            case 'boolean' :
            $result = $param_type['boolean'];
                break;
            case 'array' :
            $result = $param_type['array'];
                break;
            case 'json' :
            $result = $param_type['json'];
                break;
            case 'object' :
            $result = $param_type['object'];
                break;
            case 'number' :
            $result = $param_type['number'];
                break;
            default :
                $result = $param_type['string'];
        }
    ...
    private function isSwaggerSchemaType($arr)
    {
        if($arr['schema'])
        {
            return true;
    ...
    private function isSwaggerSchemaModel($arr)
    {
        if($arr['schema'] && $arr['schema']['$ref'])
        {
            return true;
        }
        return false;
    ...
    private function isSwaggerSchemaArray($arr)
    {
        if($arr['schema']['type'] && $arr['schema']['type']=="array")
        {
            return true;
        }
        return false;
    ...
    private function getSwaggerDefinitionsKey($arr)
    {
        $refstr='';
        if($arr['$ref'])
        {
        ...
        else if($arr['items']['$ref'])
        {
            $refstr=$arr['items']['$ref'];
        }
        else
        {
            return false;
        ...
        $indexOf=strpos($refstr,'#/definitions/');
        $startIndex=strlen('#/definitions/');
        if($indexOf == 0)
        {
            $strModel=substr($refstr,$startIndex);
            return $strModel;
        }
        return false;
    }

    private function getSwaggerModelParams($strModel,$project_definitions_list,$prefixStr)
    {
        $i=0;
        if($project_definitions_list[$strModel]['properties'])
        {
            $apiInfo = array();
            $model_properties_list=$project_definitions_list[$strModel]['properties'];
            ...
            foreach($model_properties_list as $name=>$value)
            {
                $apiInfo[$i]['paramKey'] = $prefixStr.$name;
                $apiInfo[$i]['paramName'] = $value['description'];
                $apiInfo[$i]['paramNotNull'] =$value['required']?0:1;
                $apiInfo[$i]['paramValue'] = '';
                if($value['enum'])
                {
                    $apiInfo[$i]['paramValueList'] = array();
                    $j=0;
                    foreach($value['enum'] as $v)
                    {
                        $apiInfo[$i]['paramValueList'][$j]=array("value"=>$v,"valueDescription">'');
                        ++$j;
                    }
                }
                ...
                if($value['$ref'])
                {
                    $apiInfo[$i]['paramType'] = $this->getSwaggerParamType("object");
                    $strType='object';
                    $is_model=true;
                }
                else
                {
                    $strType=$value['type'];
                    $apiInfo[$i]['paramType'] = $this->getSwaggerParamType($value['type']);
                }
                ...
                {
                    $strkey= $this->getSwaggerDefinitionsKey($value);
                    if($strkey)
                    {     
                        $paramValues=$this->getSwaggerModelParams($strkey,$project_definitions_list,$prefixStr.$name);
                        if($paramValues)
                        {
    ...
    public function importSwagger(&$content)
    {
        ...
        // 项目类型默认web
        $project_type = '0';
        $project_definitions_list = $swagger['definitions'];
                        ...
                        foreach ($api_info_list as $api_request_type => $api_info) {
                            ...
                            if ($api_info['parameters']) {
                                ...
                                foreach ($api_info['parameters'] as $param) {
                                    ...
                                    if ($param['schema'])
                                    {
                                        $strkey= $this->getSwaggerDefinitionsKey($param['schema']);
                                        if($strkey)
                                        {
                                            $paramValues=$this->getSwaggerModelParams($strkey,$project_definitions_list,$param['name']);
                                            if($paramValues)
                                            {
                                                foreach($paramValues as $v)
                                                {
                                                    $apiInfo['requestInfo'][$i]=$v;
                                                    $i++;
                                                }
                                            }
                                        }
                            ...
                            if ($api_info['responses']) {
                                ...
                                foreach ($api_info['responses'] as $paramKey => $respon) {
                                    ...
                                    if ($is_model)
                                    {
                                        $strkey= $this->getSwaggerDefinitionsKey($respon['schema']);
                                        if($strkey)
                                        {
                                            $paramValues=$this->getSwaggerModelParams($strkey,$project_definitions_list,$paramKey);
                                            if($paramValues)
                                            {
                                                foreach($paramValues as $v)
                                                {
                                                    $apiInfo['resultInfo'][$k]=$v;
                                                    $k++;
                                                }
                                            }
                                        }
